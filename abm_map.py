...
/* small fixed credit (remplace le log dynamique) */
#log{
  position: absolute;
  right: 10px;
  bottom: 10px;
...
#log a { color: #0b66c3; text-decoration: none; font-weight: 600; }
...
<div class="control-panel" id="controls">
  ...
  <div class="filter-block"><strong>Size</strong><br/>
  ...
  <div class="filter-block"><strong>Opacity range</strong><br/>
    Opacity: <input id="range_op" type="range" min="0" max="1" step="0.05" value="0.25"/> <span id="op_val">0.25</span>
  </div>
    <div class="filter-block">
    <strong>Affichages</strong><br/>
    <label><input id="show_connections" type="checkbox" checked/> Show id↔id lines</label><br/>
    <label><input id="show_orbits" type="checkbox" checked/> Show satellite orbites</label><br/>
    <label><input id="animate_sat" type="checkbox" checked/> Animate satellites & ships</label><br/>
    <label><input id="show_all_ranges" type="checkbox" /> Show all RADAR ranges</label><br/>
  </div>
  ...
  <div class="filter-block">
    ...
    <div class="small">Israeli system — placement only inside Israel or Germany</div>
  </div>
...
<div id="missile_modal" class="modal" role="dialog" aria-modal="true" aria-hidden="true">
  ...
  <div class="row">
    <label for="missile_place_input">Place (autocompletion):</label>
...
<div id="log" aria-hidden="false">
  Map developed by <a href="https://github.com/Lebesgue01" target="_blank" rel="noopener noreferrer">@Lebesgue01</a>
</div>
...
function log(msg){
  // logs still available in browser console for debugging, but not shown on the map UI
  try { console.log(new Date().toISOString() + " - " + msg); } catch(e) {}
}
...
function buildSubtypeControls(){
  ...
  // wrapper details (collapsed by default)
  ...
  if(arr.length === 0){
    inner.innerHTML = "<div class='small'>(aucun subtype trouvé)</div>";
    subContainer.appendChild(wrapper);
    return;
  }
  arr.forEach(s => {
    const wrapperChk = document.createElement('div');
    wrapperChk.innerHTML = `<label><input type="checkbox" class="subtype" value="${s}" checked/> ${s}</label>`;
    inner.appendChild(wrapperChk);
  });
  subContainer.appendChild(wrapper);
}
...
const ARROW_ALLOWED = ['israel','germany'];
const DAVID_ALLOWED = ['israel'];
...
document.getElementById('arrow_place_confirm').addEventListener('click', function(){
  ...
  // must be in Israel
  if (!isNameAllowedFor(name, ARROW_ALLOWED)) {
...
document.getElementById('david_place_confirm').addEventListener('click', function(){
  ...
  // must be in Israel
  if (!isNameAllowedFor(name, DAVID_ALLOWED)) {
...
createADMarker({
  ...
  ranges: [
    ...
    {
  radiusMeters: 40000,
  color: 'red',            // ← bordure rouge visible
  fillColor: '#f7e9a7',    // ← même couleur intérieure que le grand disque
  fillOpacity: 0.22,
  label: 'Interception (40 km)'
}
  ],
  layer: davidLayer
});
